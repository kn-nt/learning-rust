...
use crate::misc::{log, sleep, window};
    ...
    match serde_json::to_string(&p) {
        Ok(payload) => {
            match ws.send_with_str(&payload) {
                Ok(_) => log(&format!("Request for {:?}", &p)),
                Err(e) => log(&format!("Unable to request {:?}, Err {:?}", &p, e)),
            };
        }
        Err(e) => {
            log(&format!("Unable to serialize {:?}, Err {:?}", &p, e));
